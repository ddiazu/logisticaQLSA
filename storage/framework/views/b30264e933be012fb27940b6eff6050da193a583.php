        ...
        <div class="panel-body">
            ...
            <div class="table-responsive">
                <table id="tablaAprobados" class="table table-condensed" style="width: 100%">
                    <thead>
                        <th style="width:100px">Pedido Nº</th>
                        <th style="width:100px">Guia º</th>
                        <th style="width:100px">Tipo Guía</th>
                    ...
                    <tbody>
                        <?php $__currentLoopData = $pedidos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr>                                    
                                <td style="width:100px"><?php echo e($item->idPedido); ?></td>
                                <td style="width:100px"><?php echo e($item->numeroGuia); ?></td>
                                <?php if($item->tipoGuia==1): ?>
                                    <td style="width:100px">Electrónica</td>
                                <?php else: ?>
                                ...
                                <td>
                                    <?php if( $item->certificado=='' ): ?>
                                        <button class="btn btn-warning btn-sm" style="width: 60px" onclick="abrirModalSubirArchivo(this.parentNode.parentNode.rowIndex, <?php echo e($item->tipoGuia); ?> , <?php echo e($item->prod_codigo); ?> );">Subir</button>
                                    <?php else: ?>
                                        <a href="<?php echo e(asset('/')); ?>bajarCertificado/<?php echo e($item->certificado); ?>" class='btn btn-sm btn-success' style='width: 60px'>Bajar</a><button class='btn btn-danger btn-sm' style='width: 60px'>Quitar</button>   
                                    <?php endif; ?>
                                </td>
                            </tr>
        ...
        <div id="bodyProducto" class="modal-body">
            <form id="datos" name="datos" enctype="multipart/form-data">
                ...
                <div class="row" style="padding-top: 5px">
                    ...
                    <div class="col-md-7">
                        <input class="form-control input-sm" id="tipoGuia" data-tipo="0" readonly>
                    </div>
                </div>
                <div class="row" style="padding-top: 5px">
                    <div class="col-md-3">
...
<?php $__env->startSection('javascript'); ?>
    ...
    <script>
        ...
        function abrirModalSubirArchivo(fila, tipoGuia, codProducto){
            ...
            $("#filaTabla").val(fila);
            $("#numeroGuia").val(tabla.rows[fila].cells[1].innerHTML.trim() );
            $("#tipoGuia").val(tabla.rows[fila].cells[2].innerHTML.trim() );
        ...
        $('#datos').on('submit', function(e) {
          ...
          $.ajax({
              url: 'http://localhost:8000/' + "subirCertificado",
              ...
              success:function(data){
                  ...
                  tabla.rows[fila].cells[4].innerHTML="<a href="+ urlApp + "bajarCertificado/" + data + " class='btn btn-sm btn-success' style='width: 60px'>Bajar</a><button class='btn btn-danger btn-sm' style='width: 60px'>Quitar</button>";  
              },
