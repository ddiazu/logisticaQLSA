    ...
    //var urlApp='http://svdev.ddns.net:88/qlsa/public/';
...
//Creamos la variable para detectar el navegador
  ...
  function verUbicacionGmaps(patente){
      var sessionID="";
      var secretKey="";
        $.ajax({
            async: true,
            url: urlApp + "solicitarSessionIDSitrack" ,
            ...
            success:function(dato){
              posicion='https://www.google.com/maps/search/?api=1&query=' + dato[0].latitude +', ' + dato[0].longitude + '&map_action=map';
              window.open(posicion, "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=100,left=100,width=1200,height=600" );              
            }
        })
...
function downloadCSV(csv, filename) {
    ...
    function fechaIngles(fecha){
      if(fecha.trim()==''){
          ...
          return fecha.substr(4,4)+'-'+fecha.substr(2,2)+'-'+fecha.substr(0,2);
      }
    }

    function fechaHoraATexto(fechaHora){
      var fecha=fechaHora.substr(0,10);
      return fechaAtexto(fecha)+fechaHora.substr(11);
    }